...
class State(object):
    ...
    def transition(self,changes,trace,therblig,agent,tofs,agents):
        ...
        if "grasping" in local_therblig["sets"].keys() and agents[agent]["type"] == "robot":
            time += math.fabs(self.grasp(agent)-changes["gripping"][agent]) # Assumes gripper will close in 1 second max
